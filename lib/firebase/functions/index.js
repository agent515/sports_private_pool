            ...
            if (result.data() === undefined) {
                            ...
                            return admin.firestore().doc(`/contests/cricketMatchContest/cricketMatchContestCollection/${contestId}`).get()
                                .then((contestData) => {
                                    ...
                                    for (let key in contest['predictions']) {
                                        ...
                                        result[key] = matchResult['batting'][value['mostRuns']] + matchResult['bowling'][value['mostWickets']] + matchResult['mvp'][value['MVP']];
                                    ...
                                    admin.firestore().doc(`/contests/cricketMatchContest/cricketMatchContestCollection/${contestId}`).update({ result: resultObjectArray }).then((_) => console.log(_)).catch(e => console.log(e));
                                    ...
                                    for (let i = 0; i < winners.length; i++) {
                                        admin.firestore().doc(`users/${winners[i]}`).get()
                                            .then(winnerUser => {
                                                let currPurse = winnerUser['purse'];
                ...
                return admin.firestore().doc(`/contests/cricketMatchContest/cricketMatchContestCollection/${contestId}`).get()
                    .then((contestData) => {
                        ...
                        for (let key in contest['predictions']) {
                            let value = contest['predictions'][key];
                            console.log(matchResult['batting'][value['mostRuns']]);
                            console.log(typeof (matchResult['batting'][value['mostRuns']]));
                            result[key] = parseInt(matchResult['batting'][value['mostRuns']]['percentile']) + parseInt(matchResult['bowling'][value['mostWickets']]['percentile']) + parseInt(matchResult['mvp'][value['MVP']]['percentile']);
                            if (matchResult['winner'] != undefined) {
                                if (matchResult['winner'] == 'draw' && value['matchResult'] == matchResult['winner']) {
                                    result[key] += 150.0;
                                }
