                            ...
                            return admin.firestore().doc(`/contests/cricketMatchContest/cricketMatchContestCollection/${contestId}`).get()
                                .then((contestData) => {
                                    ...
                                    for (let key in contest['predictions']) {
                                        ...
                                        result[key] = parseDouble(matchResult['batting'][value['mostRuns']]['percentile']) + parseDouble(matchResult['bowling'][value['mostWickets']]['percentile']) + parseDouble(matchResult['mvp'][value['MVP']]['percentile']);
                    ...
                    .then((contestData) => {
                        ...
                        for (let i = 0; i < winners.length; i++) {
                            admin.firestore().doc(`users/${winners[i]}`).get()
                                .then(winnerUser => {
                                    let currPurse = winnerUser['purse'];
                                    admin.firestore().doc(`users/${winners[i]}`).update({ purse: parseDouble(currPurse) + distributeMoney });
                                })
                                .catch(e => console.log(e));
                        }
